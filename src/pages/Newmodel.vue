<template>
  <div class="container">
    <div class="form">
      <h1 class="title is-2 has-text-centered">创建模型集</h1>
      <div class="field">
  ...
  methods: {
    ...
    createDataset() {
      ...
      for (let labelId of this.labels) {
        modelset.labels.push(this.$refs[`label${labelId}`].getValue());
      }
      this.$http
        .post("/api/modelsets", modelset)
        .then((res) => {
          console.log(res.data);
          this.$router.push("/modelsets/" + res.data.id);
        })
        .catch((err) => {
